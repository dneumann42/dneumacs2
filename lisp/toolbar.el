...
See the Commentary for the accepted item forms."
  (concat
   ...
   ;; header line.  Exactly one character, so callers that compose
   ;; several sections (see doc-toolbar) can strip it with a
   ;; one-character `substring'.  Stop one pixel short of the edge: a
   ;; line that exactly fills a `truncate-lines' window still gets a
   ;; truncation glyph drawn over its last column.
